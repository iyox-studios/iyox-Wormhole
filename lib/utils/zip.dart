...
Stream<ZipProgress> zipFolder(
    String folderUri, List<DocumentFile> files) async* {
  final archive = Archive();
  final folderName =
      Uri.parse(folderUri).pathSegments.last.split(':').last.split('/').last;
  final totalFiles = files.length;
  int processedFiles = 0;

  for (final file in files) {
    final fileName = Uri.parse(file.uri)
        .pathSegments
        .last
        .split(':')
        .last
